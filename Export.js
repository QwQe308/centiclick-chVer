...
function Handle(decodedData){
    ...
    // console.log(pts);
    // console.log(Number(pts));
    // console.log(pts===Number(pts));
    ...
    t6=Number(t6);
    pts=Number(pts);
    lastpt=Number(lastpt);
    bestpt=Number(bestpt);
    lastgl=Number(lastgl);
    bestgl=Number(bestgl);
    glitch=Number(glitch);
    glitchcount=Number(glitchcount);
    glitchpower=Number(glitchpower);
    hidecompleted==Number(hidecompleted);
    currentchal=Number(currentchal);
    stage=Number(stage);
    apoint=Number(apoint);
    bpoint=Number(bpoint);
    syscount=Number(syscount);
    loc=Number(loc);
    wp=Number(wp);
    tcyc=Number(tcyc);
	tl = 3000;
